...
// Loading Screen y reveal del body
window.addEventListener('load', () => {
    ...
    setTimeout(() => {
        ...
        // 2. Revelar el body (estaba en opacity:0 por CSS)
        ...
        // 5. Arrancar animaciones de entrada de los menu-items
        //    Les quitamos y ponemos 'initial-load' para resetear el keyframe
        //    ahora que el body ya es visible.
        const items = document.querySelectorAll('.menu-item.initial-load');
        items.forEach(item => {
            item.classList.remove('initial-load');
            void item.offsetWidth; // force reflow para resetear la animación
...
function filterGallery(category, btn) {
    ...
    items.forEach(item => {
        // Obtenemos las categorías del item y las convertimos en un array (separando por espacios)
        const itemCategories = item.dataset.category.split(' ');

        // Verificamos si es 'all' o si el array de categorías del item incluye la categoría seleccionada
